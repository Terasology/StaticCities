...
/**
 * Generates a {@link TownWall} around a given settlement while respecting blocked areas
 */
...
public class TownWallFacetProvider implements FacetProvider {
        ...
        for (Settlement settlement : settlementFacet.getSettlements()) {
            if (settlement.hasTownwall()) {
                ...
                if (Circle.intersects(site.getPos(), site.getRadius(), wallFacet.getWorldRegion())) {
                    try {
                        TownWall tw = cache.get(site, () -> generate(site, heightFacet, buildableAreaFacet,
                                blockedAreaFacet));
    ...
    private TownWall generate(Site city, InfiniteSurfaceHeightFacet heightFacet,
                              BuildableTerrainFacet buildableAreaFacet, BlockedAreaFacet blockedAreaFacet) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (thisOk) {
                ...
                if (!prevOk) {            // the previous location was blocked -> this is the second part of a gate
                    ...
                    lastHit = i;
                    lastTower = tower;
                } else if (!nextOk) {            // the next location is blocked -> this is the first part of a gate
                    SimpleTower tower = createGateTower(orient, heightFacet, towerPos);
                    tw.addTower(tower);
                    ...
                    lastHit = i;
                    lastTower = tower;
                } else if (distToTower(towerPos, firstTower) > minDist && distToTower(towerPos, lastTower) > minDist) {
